...
function numberOfCharacters(string) {
  ...
  for (var i = 0; i < string.length; i++) {
    ...
    if (frecuencias.hasOwnProperty(letra)) {
      frecuencias[letra]++;
    } else {
      frecuencias[letra] = 1;
    }
  }
  var letrasOrdenadas = Object.keys(frecuencias).sort()
  var resultado = {};
  for (var i = 0; i < letrasOrdenadas.length; i++) {
    var letraOrdenada = letrasOrdenadas[i];
    resultado[letraOrdenada] = frecuencias[letraOrdenada];
  }
  return resultado;
}
...
function capicua(numero) {
  ...
  var cadenaRevertida = cadenaOriginal.split("").reverse().join("");

  if (cadenaOriginal === cadenaRevertida) {
    return "Es capicua";
  } else {
...
function deleteAbc(string) {
  ...
  // Tu código:
  var resultado = string.split('').filter(letra => !['a', 'b', 'c'].includes(letra)).join('');
  return resultado;
}
